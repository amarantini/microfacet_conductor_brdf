...
if (PART < 3)
  ...
  for (SceneLight* light : scene->lights) {
    ...
    for (int i = 0; i < num_light_samples; i++) {
      ...
      const Spectrum& light_L = light->sample_L(hit_p, &dir_to_light, &dist_to_light, &pdf);
      const Vector3D& w_in = w2o * dir_to_light;
      if (w_in.z < 0) continue;

      if (!bvh->intersect(Ray(hit_p + EPS_F * dir_to_light, dir_to_light, dist_to_light * (1 - EPS_F * 2)))) {
        double cos_theta = w_in.z;
        const Spectrum& f = isect.bsdf->eval(w_out, w_in);
        L_out += (cos_theta / (num_light_samples * pdf)) * f * light_L;
      }
    }
  }  
  return L_out;
...
if (PART < 4)
  ...
  float pdf;
  Vector3D w_in;
  const Spectrum& f = isect.bsdf->sample(w_out, &w_in, &pdf);
  
  // Russian Roulette - 
  // need a multiplier to make the illum() a reasonable probability to reduce noise
//  const float multiplier = 10.f;
//  float reflectance = clamp(0.f, 1.f, f.illum() * multiplier + 0.05f);

  float reflectance = 0.95f;

  float terminate_prob = 1.f - reflectance;
  if (coin_flip(terminate_prob)) return Spectrum();
  ...
  return scale * f * trace_ray(rec, isect.bsdf->is_delta());
}
...
//            Ray r = camera->generate_ray_for_thin_lens(p.x/sampleBuffer.w, p.y/sampleBuffer.h, samplesForLens.x, samplesForLens.y * 2.0 * M_PI);
...
//           Ray r = camera->generate_ray_for_thin_lens(p.x/sampleBuffer.w, p.y/sampleBuffer.h, samplesForLens.x, samplesForLens.y * 2.0 * M_PI);
            ...
            r.depth = max_ray_depth;
            Spectrum current = trace_ray(r,true);
            s += current;
            float currentIllum = current.illum();
            s1 += currentIllum;
            s2 += currentIllum * currentIllum;

            if (i > 0 && i % samplesPerBatch == 0) {
                float n = i + 1.0f;
                float mean = s1 / n;
                float stdev = std::sqrt((s2 - s1 * s1 / n) / (n - 1.0f));

                float CI = 1.96f * stdev / std::sqrt(n);

                if (CI <= maxTolerance * mean) {
                    sampleCountBuffer[y * sampleBuffer.w + x] = (int) n;
                    return s * (1.0f / n);
                }
            }
        }
        sampleCountBuffer[y * sampleBuffer.w + x] = num_samples;
        return s * (1.0f / num_samples);
    }
